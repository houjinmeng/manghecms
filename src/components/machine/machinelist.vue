<template>
  <div>
    ...
    <van-search placeholder="请输入编号、设备名称、地址" v-model="value" @blur="onSearch" />
    <div class="status">
      <!-- <div class="title">设备状态</div> -->
      <div class="btn">
        <div class="one">全部20</div>
        <div class="one">正常使用4</div>
        <div class="one">故障16</div>
    ...
    <div style="padding:0 0.2rem">
      ...
      <div class="list1" v-show="ziying">
        <div class="title">
          ...
          <span style="font-size:0.3rem">在线{{ziyingon}}/{{ziyingnum}}台</span>
        </div>
        <div class="item" @click="setmachine(item)" v-for="(item,index) in list" :key="index">
          <div style="display:flex;align-items:center;">
            <span style="display:inline-block;width:3.6rem">{{item.machine_name}} ></span>
            <span :class="[ item.is_online=='在线' ? 'online' : 'outline' ]"></span>
            {{item.is_online}}
          </div>
          <div style="display:flex;align-items:center;">
            <van-icon
      ...
      <div class="list1" v-for="(item,index) in zuzhi" :key="index">
        <div class="title">
          ...
          <span style="font-size:0.3rem">{{zuzhion[index]}}/{{item.length}}台</span>
        </div>
    ...
    <van-popup v-model="show" position="bottom">
      <div class="dialog">
        <div class="title">
          <div class="top">
            <span class="left">迪士尼-万达-望京-设备1</span>
            <span style="color:red">{{status}}</span>
          </div>
          <div style="font-size:0.28rem;text-align:left">地址:北京市朝阳区</div>
        </div>
...
export default {
  data() {
    return {
      ...
      show: false,
      editdialog: false,
      options: [{ value: 0, label: "正常" }, { value: 1, label: "故障" }],
      ziyingnum: "",
      ziyingon: "",
      zuzhion: []
    };
