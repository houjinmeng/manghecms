<template>
  <div>
    ...
    <van-search v-model="data.key" placeholder="请输入搜索关键词" show-action shape="round" @search="onSearch">
      <div slot="action" @click="onSearch" style="color:#8598d5;font-size:0.35rem">搜索</div>
    </van-search>
    <div class="box" @click="addgoods(item.machine_id)" v-for="(item,index) in list" :key="index">
      <div class="top">
...
export default {
  ...
  methods: {
    // 获取设备列表
    onSearch() {
      this.$http.post('/delivery/machine_list',this.$qs.stringify(this.data)).then(res=>{
        this.list=res.data
      })
    },
    // 补货清货
